...
def main():
    ...
    if training_args.do_train:
        train_original_dataset = load_dataset("MahmoudAly/LexAbSumm", split="train", cache_dir=model_args.cache_dir)
        # train_dataset = train_original_dataset.remove_columns([col for col in train_original_dataset.column_names if col != "law_summary"])

        
        # Split the dataset into 80% for initial train and 20% for temporary set
        split_dataset = train_original_dataset.train_test_split(test_size=0.2)
    
        train_dataset = split_dataset['train']  # final training set
        temp_dataset = split_dataset['test']    # This will be split further into validation and test
    
        # Split the temporary set into 50% validation and 50% test
        final_split = temp_dataset.train_test_split(test_size=0.5)
        val_dataset = final_split['train']  # validation set
        test_dataset = final_split['test']  # test set

        # print(train_dataset[0]['law_summary']) #测试是否正确预测

    if training_args.do_eval:
        eval_dataset = val_dataset
        # print(eval_dataset[0]['law_summary'])

    if training_args.do_predict:
        predict_dataset = test_dataset
        # print(predict_dataset[0]['law_summary'])

    # Labels
    label_list = list(range(1,18))
    ...
    def filter_strings_starting_with_numbers(string_list):
        """
        Filter a list of strings to keep only those starting with a number.

        Args:
        string_list (list): A list of strings.

        Returns:
        list: A filtered list containing only strings starting with a number.
        """
        filtered_list = [s for s in string_list if s[0].isdigit()]
        return filtered_list

    def preprocess_function(examples):

        # for entry in examples:
        #     if 'facts_source' in entry:
        #         entry['facts_source'] = filter_strings_starting_with_numbers(entry['facts_source'])
        ...
        if model_args.hierarchical:
            ...
            if config.model_type == 'roberta':
                ...
                for case in examples['facts_source']:
        ...
        # Check if 'labels' key exists in the batch, if not, initialize it
        if "labels" not in batch:
            batch["labels"] = []

        for summary in examples["law_summary"]:
        # Find all occurrences of "violation of Article" followed by a number
            found_labels1 = set(map(int, re.findall(r"violation of Article (\d+)", summary, re.IGNORECASE)))
        
        # Find all occurrences of "violations of Article" followed by a number
            found_labels2 = set(map(int, re.findall(r"violations of Article (\d+)", summary, re.IGNORECASE)))
        
        # Find all occurrences of "violation of Articles" followed by a number
            found_labels3 = set(map(int, re.findall(r"violation of Articles (\d+)", summary, re.IGNORECASE)))

        # Combine all found labels
            found_labels = found_labels1.union(found_labels2).union(found_labels3)
        
        # Check if 'no' appears before any of the label phrases
            if re.search(r"no violation of Article (\d+)", summary, re.IGNORECASE):
                no_labels = set(map(int, re.findall(r"no violation of Article (\d+)", summary, re.IGNORECASE)))
                found_labels -= no_labels
        
        # Convert the labels to one-hot encoding
            one_hot_encoded_labels = [1 if label in found_labels else 0 for label in label_list]
            batch_labels.append(one_hot_encoded_labels)
